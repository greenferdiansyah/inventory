...
class Jenis extends CI_Controller {
	...
	function __construct() {
		...
		$this->access 		= $this->page_render->page_auth_check('jenis');
	...
	public function json_list(){   //server side mode for data tabel use only //
			...
			$data 			= $this->m_jenis->list_data($length,$start,$search,$order,$dir);
	...
	public function form(){
        ...
        if($action != 'Edit'){
                ...
                $kategori_id            = null;
					...
					$result			        = $this->m_jenis->detail_data($id);
                    $title					= 'Edit Jenis';
				...
				//$this->drop_down->where("option_type = 'opt_status'");
				$this->drop_down->order("id_kategori", "ASC");
	...
	public function form_submit(){
		...
		$response	= $this->m_jenis->submit($action, $data);
	...
	public function delete(){
		...
		$response	        = $this->m_jenis->delete($id_jenis);
		echo json_encode(array("status"=> $response["status"], "title"=>$response["title"], "reason"=> $response["reason"]));
	}
}
