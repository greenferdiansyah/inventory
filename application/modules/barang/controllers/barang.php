...
class Barang extends CI_Controller {
	...
	public function json_list(){
			...
			foreach ($data['data'] as $row) {
				...
				$button_action = "<center>
					...
					<a onclick=show_detail('".$id."') id =$id class='btn btn-info btn-xs' title='detail' style='color: white;'><i class='fa fa-pencil'></i></a></center>";
	...
	public function modal(){
		...
		if($this->session->userdata('is_logged_in') == true){
			if($this->access != 0){
					...
					$result 						= $this->m_barang->detail_data($id);
					$send['id_barang'] 				= $result->id_barang;
					$send['kode_barang']			= $result->kode_barang;
					$send['nama_barang'] 			= $result->nama_barang;
					$send['vendor_id'] 				= $result->vendor_id;
					$send['kategori_id']			= $result->kategori_id;
					$send['tipe_id']				= $result->tipe_id;
					// $send['jenis_id']				= $result->jenis_id;
					// $send['merk_id']				= $result->merk_id;
					$send['satuan_id']				= $result->satuan_id;
					$send['stock']					= $result->stock;
					$send['deskripsi']				= $result->deskripsi;
	...
	// public function modal(){
	// 	$id_barang 			= $this->m_barang->detail_data($id_barang);
	...
	public function delete(){
		...
		$response	        = $this->m_barang->delete($id_barang);
		echo json_encode(array("status"=> $response["status"], "title"=>$response["title"], "reason"=> $response["reason"]));
	}
}
