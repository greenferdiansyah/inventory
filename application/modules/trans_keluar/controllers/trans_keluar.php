...
class Trans_keluar extends CI_Controller {
	...
	function __construct() {
		...
		$this->access 		= $this->page_render->page_auth_check('trans_keluar');
	...
	public function json_list(){
			...
			$data 			        = $this->m_trans_keluar->list_data($length,$start,$search,$order,$dir);
	...
	public function form(){
        ...
        if($action != 'Edit'){
				...
				$result			        = $this->m_trans_keluar->detail_data($id);
                $title					= 'Edit Barang Keluar';
                $id_trans_keluar        = $result->id_trans_keluar;
                $barang_id	    		= $result->barang_id;
                $tenant_to 				= $result->tenant_to;
	...
	public function form_submit(){
		...
		$data         = array(
                                ...
                                'tenant_to'             => $this->input->post("tenant_to"),
		...
		$response	= $this->m_trans_keluar->submit($action, $data);
	...
	public function delete(){
		$id_trans_keluar    = $this->input->post("id");
		$response	        = $this->m_trans_keluar->delete($id_trans_keluar);
		echo json_encode(array("status"=> $response["status"], "title"=>$response["title"], "reason"=> $response["reason"]));
	}
}
